<template>
  ...
  <body>
  <HeaderComp/>
  <div class="container py-4 py-xl-5" style="min-height: 100vh;">
    <div class="row mb-5">
      <div class="col-md-8 col-xl-6 text-center mx-auto">
        <h2>{{ Tip.heading }}</h2>
        <br>
        <p class="w-lg-50 text-sm-start">{{ Tip.description }}</p>
        <br>
        <button type="button" id='copyButton' @click="copyText(Tip.code)" class="btn btn-outline-primary"
                style="float: right;">Copy code
        </button>
        <code-highlight language="">
          <div id='codePlaceToCopy' style="color: #eeece7 !important;">{{ Tip.code }}</div>
        </code-highlight>
        ...
        <em>
          <div class="container" style="font-size: 14px;">
            ...
            <div class="row justify-content-end">
              <div v-if="Tip.tags" class=' col col-lg-2'
              ...
              <div v-if="Tip.dateCreated" style="margin: 10px 0 10px 0" class='col col-lg-3 d-flex'>
                Date of creation {{ Tip.dateCreated.split('T')[0] }}
                {{ Tip.dateCreated.split('T')[1].split('.')[0].split(':').slice(0, 2).join(':') }}
              </div>

              <div v-if="Tip.dateModified" style="margin: 10px 0 10px 0" class='col col-lg-3 d-flex'>
...
import "../assets/detailpage/bootstrap/css/bootstrap.min.css"
import "../assets/detailpage/css/Navbar-Right-Links-Dark.css";
import "../assets/detailpage/css/Pricing-Free-Paid.css";
import "../assets/detailpage/css/styles.css";
  ...
  methods: {
    ...
    GetTip() {
      ...
      const SEARCH_QUERY = gql`
         query tipByHash{
          tipByHash(urlHash: "${this.$route.params.TipHash}") {
          heading
          description
          code
          dateCreated
      ...
      this.Tip = computed(() => result.value?.tipByHash ?? [])
      console.log(result)
